...
5. [Crazyflie firmware for mellinger velocity control] `crazyflie-firmware` at https://bitbucket.org/nusuav/cfl_crazyflie-firmware (Matthew's fork at https://github.com/matthewoots/crazyflie-firmware)
...
git clone git@bitbucket.org:nusuav/cfl_apriltag_msgs.git apriltag_msgs --branch safmc2024/master
git clone git@bitbucket.org:nusuav/cfl_apriltag_ros.git apriltag_ros --branch safmc2024/master
git clone git@bitbucket.org:nusuav/cfl_crazyswarm2.git crazyswarm2 --branch safmc2024/master --recursive
...
git clone git@bitbucket.org:nusuav/cfl_crazyswarm2_application.git crazyswarm2_application --branch safmc2024/master --recursive
...
## Branches and Tags for SAFMC 2024

To manage the branches for SAFMC 2024 and ensure that the correct version of code is being deployed, strictly follow the guidelines for branching below:

- `safmc2024/master`: Master branch for merged and **tested** code
- `safmc2024/<name>/master`: Individual branch for development
- `safmc2024/<name>/<feature_or_bugfix>` Individual branch for your own features, bugfixes if needed

Merging should follow the sequence: `safmc2024/<name>/<feature_or_fix>` → `safmc2024/<name>/master` →
`safmc2024/master`.

Typically, you will do your development under `safmc2024/<name>/master` or `safmc2024/<name>/<feature_or_fix>`. After integration and testing, it will be merged to `safmc2024/master` and have a tag added (see below). This is to ensure that we always have a reliable working version for both teams. If two or more people are working on the same code, they can also merge from `safmc2024/master` to their own branch `safmc2024/<name>/master` and be assured that the code is working and tested.

For tags, use the following format:

- `test/<yymmdd>`
- `demo/<yymmdd>`

After every integration test, add this tag to the commit that completed the test or demo successfully so that we will always know which is the last working version. When something breaks, we can easily see the changes that caused the issue by comparing the current commit with the last working version.
